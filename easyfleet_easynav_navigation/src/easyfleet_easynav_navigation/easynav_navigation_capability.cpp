...
#include "easyfleet_easynav_navigation/easynav_navigation_capability.hpp"
...
: easyfleet_core::NavigationActionServerBase(node, action_name)
{
  ...
  // observed to make the array-typed waypoint_ids load empty.
  ...
  factory_.registerNodeType<StartOff>("StartOff");
  factory_.registerNodeType<Finish>("Finish");
  factory_.registerNodeType<Navigate>("Navigate", waypoints_, gm_client_);
}
