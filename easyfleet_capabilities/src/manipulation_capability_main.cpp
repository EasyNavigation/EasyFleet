...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<manipulation_capability::ManipulationCapability>();

  node->configure();
  if (node->get_current_state().id() != lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE) {
    RCLCPP_FATAL(node->get_logger(), "manipulation_capability failed to configure, exiting.");
    rclcpp::shutdown();
    return 1;
  }
  ...
  if (node->get_current_state().id() != lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {
    RCLCPP_FATAL(node->get_logger(), "manipulation_capability failed to activate, exiting.");
    rclcpp::shutdown();
    return 1;
  }
