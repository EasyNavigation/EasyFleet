...
: arch_mockup::ActionServerBase<nav2_msgs::action::NavigateToPose>(node, action_name)
{
...
rclcpp_action::GoalResponse NavigateToPoseActionServer::on_goal_received(
...
void NavigateToPoseActionServer::on_execute(const GoalHandleSharedPtr goal_handle)
{
  ...
  feedback->current_pose = goal->pose;
  ...
  for (int i = 0; i < num_steps; ++i) {
    ...
    if (is_shutdown_requested()) {
      auto result = std::make_shared<Result>();
      result->error_code = Result::UNKNOWN;
    ...
    feedback->position_tracking_error = 0.0f;
    feedback->heading_tracking_error = 0.0f;
