...
bool requests_target_class(
  const std::vector<std::string> & objects, const std::string & target_class)
{
  return std::any_of(
...
: arch_mockup::ActionServerBase<perception_interfaces::action::Perception>(node, action_name),
...
{
  if (goal->objects.empty() || !requests_target_class(goal->objects, target_class_)) {
