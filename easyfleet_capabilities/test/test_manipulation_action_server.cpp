...
using manipulation_capability::ExecuteTrajectoryActionServer;
...
moveit_msgs::msg::RobotTrajectory make_trajectory()
{
  ...
  trajectory.joint_trajectory.points.push_back(start);

  trajectory_msgs::msg::JointTrajectoryPoint end;
  end.positions = {1.0, -1.0};
  end.time_from_start.sec = 1;
  trajectory.joint_trajectory.points.push_back(end);
...
class ExecuteTrajectoryActionServerTest : public ::testing::Test
{
protected:
  static void SetUpTestSuite()
  {
    ...
    action_server_ = std::make_shared<ExecuteTrajectoryActionServer>(
      server_node_.get(), action_name_);

    client_node_ = std::make_shared<rclcpp::Node>(unique_test_name("test_manip_client"));
    client_ = rclcpp_action::create_client<ExecuteTrajectory>(client_node_, action_name_);
  ...
  inline static std::shared_ptr<ExecuteTrajectoryActionServer> action_server_;
  inline static rclcpp::Node::SharedPtr client_node_;
  inline static rclcpp_action::Client<ExecuteTrajectory>::SharedPtr client_;
  inline static rclcpp::executors::SingleThreadedExecutor::SharedPtr executor_;
  inline static std::thread spin_thread_;
};

TEST_F(ExecuteTrajectoryActionServerTest, ExposesConfiguredActionName)
{
  EXPECT_EQ(action_server_->get_action_name(), action_name_);
}

TEST_F(ExecuteTrajectoryActionServerTest, RejectsGoalWithEmptyTrajectory)
{
...
TEST_F(ExecuteTrajectoryActionServerTest, AcceptsGoalAndSucceedsWithNoError)
{
  ExecuteTrajectory::Goal goal;
...
TEST_F(ExecuteTrajectoryActionServerTest, FeedbackReportsExecutingState)
{
  ExecuteTrajectory::Goal goal;
  goal.trajectory = make_trajectory();
...
TEST_F(ExecuteTrajectoryActionServerTest, ClientCancelResultsInCanceled)
{
  ExecuteTrajectory::Goal goal;
  goal.trajectory = make_trajectory();
  ...
  ASSERT_TRUE(wrapped.result);
  EXPECT_EQ(wrapped.result->error_code.val, moveit_msgs::msg::MoveItErrorCodes::PREEMPTED);
}

TEST_F(ExecuteTrajectoryActionServerTest, NewGoalPreemptsRunningGoalByDefault)
{
  EXPECT_TRUE(action_server_->is_preemptable());

  ExecuteTrajectory::Goal first_goal;
  ...
  ExecuteTrajectory::Goal second_goal;
...
TEST(ExecuteTrajectoryActionServerStandaloneTest, NonPreemptableRejectsSecondGoalWhileBusy)
{
  ...
  auto action_server = std::make_shared<ExecuteTrajectoryActionServer>(
  ...
  auto client = rclcpp_action::create_client<ExecuteTrajectory>(client_node, action_name);
