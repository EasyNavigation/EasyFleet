...
// The fake-capability half of a robot in this scenario: a single process
// hosting whichever fake capabilities this robot is configured to carry
// (see the "capabilities"/"config_subdir" parameters, e.g.
// config/robot_1/robot_params.yaml) -- only "perception" and "manipulation"
// are known here, unlike the sibling fake deployments' own robot_node,
// since this scenario's "navigation" is real EasyNav, run as a separate
// process (see launch/robot_1_launch.yaml / robot_2_launch.yaml, which
// launch this alongside easynav_system system_main +
// easyfleet_easynav_deployment's easynav_navigation_capability_node).
// Kept deliberately separate from the mission control program
// (main_easynav_collaboration.cpp / easynav_collaboration_mission_node),
// which only ever talks to capabilities over their ROS actions, never
// links against their implementation.
...
#include "easyfleet_easynav_collaboration_deployment/perception_fake_capability.hpp"
...
/// substitution -- this process may host several distinctly-named
/// lifecycle nodes, and launch_yaml's inline parameter overrides only have
/// one unambiguous target node per `node:` action, this process's exec has
  ...
  const rclcpp::Logger & logger, const std::string & name, const std::string & capabilities_file)
{
  ...
  if (name == "perception") {
    return std::make_shared<easyfleet_easynav_collaboration_deployment::PerceptionFakeCapability>(
        options);
  }
  if (name == "manipulation") {
    return std::make_shared<
      easyfleet_easynav_collaboration_deployment::ManipulationFakeCapability>(options);
  }
...
int main(int argc, char ** argv)
{
  ...
  // explicit name ("perception", "manipulation", set by its own
  // constructor), so this node just needs one distinct from theirs.
