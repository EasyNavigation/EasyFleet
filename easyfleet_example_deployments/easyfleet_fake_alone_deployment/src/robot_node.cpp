...
// easyfleet_core::Capability lifecycle node. Kept deliberately separate
// from the mission control program (main_alone.cpp / alone_mission_node),
...
#include "easyfleet_fake_alone_deployment/perception_fake_capability.hpp"
  ...
  const rclcpp::Logger & logger, const std::string & name, const std::string & capabilities_file)
{
  ...
  if (name == "navigation") {
    return std::make_shared<easyfleet_fake_alone_deployment::NavigationFakeCapability>(options);
  }
  if (name == "manipulation") {
    return std::make_shared<easyfleet_fake_alone_deployment::ManipulationFakeCapability>(options);
  }
  if (name == "perception") {
    return std::make_shared<easyfleet_fake_alone_deployment::PerceptionFakeCapability>(options);
  }
