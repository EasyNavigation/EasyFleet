...
#include "easyfleet_easynav_collaboration_simple_api_deployment/perception_fake_capability.hpp"
...
// A non-template alias, registered below, so a Deployment can load this
// backend by pluginlib lookup name ("perception", see
// easyfleet_easynav_collaboration_simple_api_deployment_plugins.xml) instead of
// needing PerceptionFakeCapability's own fixed-name constructor above --
// see easyfleet_core/capability_factory.hpp for why the factory is what's
// plugin-loaded, not the capability node itself. Registering the raw
// template instantiation directly (`easyfleet_core::CapabilityFactoryFor<PerceptionFakeActionServer>`)
// would work too, but PLUGINLIB_EXPORT_CLASS registers classes by the
// *stringified* macro argument, which must then match the plugins.xml
// `type=` attribute character-for-character (including template angle
// brackets and any incidental whitespace) -- a non-template alias avoids
// that fragility entirely.
using PerceptionFakeCapabilityFactory =
  easyfleet_core::CapabilityFactoryFor<PerceptionFakeActionServer>;

}  // namespace easyfleet_easynav_collaboration_simple_api_deployment

PLUGINLIB_EXPORT_CLASS(
  easyfleet_easynav_collaboration_simple_api_deployment::PerceptionFakeCapabilityFactory,
